import { compileEmail } from './base-layout.compiler';

/**
 * Construit le template d'email de vérification OTP en utilisant le template général Koudmain.
 */
export function buildEmailVerificationHtml(firstName: string, code: string): string {
  ...
  return compileEmail({
    ...
    leadText: `Bonjour <strong>${firstName}</strong>,<br />Utilisez le code ci-dessous pour finaliser votre inscription sur Koudmain.`,
    contentHtml,
    warningText:
